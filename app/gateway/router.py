...
class LLMRouter:
    ...
    async def _open_with_retries(
        ...
        for retry in range(dep.max_retries + 1):
            ...
            try:
                ...
                await _aclose(iterator)
                if not e.retryable:
                    breaker.record_success()  # the provider is fine; the request is not
                    attempts.append(
                        Attempt(
                            deployment=dep.name,
        ...
        try:
            while True:
                try:
                    ...
                    self._record_interruption(opened, breaker, error)
    ...
    def _record_interruption(self, opened: OpenStream, breaker: CircuitBreaker, error: str) -> None:
        dep = opened.deployment
        ...
        if breaker.record_failure():  # it did fail, even though it answered first
            metrics.BREAKER_OPEN.labels(dep.name).set(1)
